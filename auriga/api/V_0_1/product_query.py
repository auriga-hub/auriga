...
def get_product_filter_data(query_args=None):
	...
	if query_args:
		...
		page_length = cint(query_args.page_length) if query_args.get("page_length") else 6
	else:
	...
	engine = ProductQuery()
	engine.page_length=page_length
	try:
		result = engine.query(
			attribute_filters, field_filters, search_term=search, start=start, item_group=item_group, page_length=page_length
		)
	except Exception:
